...
static TAVL* TAVL_rot_dir(TAVL *t){
  ...
  t->esq = novo->dir;
  novo->dir = t;
  printf("TAVL_rot_dir -> calc_alt(t->esq) = %d , calc_alt(t->dir) = %d \n", calc_alt(t->esq), calc_alt(t->dir));
  t->alt = max(calc_alt(t->esq), calc_alt(t->dir)) + 1;
  novo->alt = max(calc_alt(novo->esq), t->alt) + 1;
  printf("TAVL_rot_dir -> calc_alt(novo->esq) = %d , t->alt = %d , novo->alt = %d \n", calc_alt(novo->esq), t->alt, novo->alt);
  TAVL_imprime(novo);
...
static TAVL* TAVL_rot_esq(TAVL *t){
  ...
  t->dir = novo->esq;
  novo->esq = t;
  printf("TAVL_rot_esq -> calc_alt(t->esq) = %d , calc_alt(t->dir) = %d \n", calc_alt(t->esq), calc_alt(t->dir));
  t->alt = max(calc_alt(t->esq), calc_alt(t->dir)) + 1;
  novo->alt = max(calc_alt(novo->dir), t->alt) + 1;
  printf("TAVL_rot_esq -> calc_alt(novo->dir) = %d , t->alt = %d , novo->alt = %d \n", calc_alt(novo->dir), t->alt, novo->alt);
  TAVL_imprime(novo);
  return novo;  /* nova raiz */
}
...
TAVL* TAVL_retira(int x, TAVL *t){     
  if(!t) return t;
    ...
    if(FB(t) == 2)
      if(FB(t->esq) >= 0){
        printf("FB(t) = %d , FB(t->esq) = %d , RSD(%d)\n", FB(t), FB(t->esq), t->info);
        t = TAVL_rot_dir(t);
      } else {
        printf("FB(t) = %d , FB(t->esq) = %d , RED(%d)\n", FB(t), FB(t->esq), t->info);
        t = TAVL_rot_esq_dir(t);
      }
  }
  else if(x < t->info){
    ...
    if(FB(t) == -2)
      if(FB(t->dir) <= 0){
        printf("FB(t) = %d , FB(t->esq) = %d , RSE(%d)\n", FB(t), FB(t->dir), t->info);
        t = TAVL_rot_esq(t);
      }else{
        printf("FB(t) = %d , FB(t->esq) = %d , RDE(%d)\n", FB(t), FB(t->dir), t->info);
        t = TAVL_rot_dir_esq(t);
      }
  }
  else{
